...
def _extract_yes_price_from_market_payload(payload: dict[str, object]) -> float | None:
    outcomes = [
        ...
        for item in _parse_json_array_field(payload.get("outcomes"))
    ]
...
def _fetch_current_yes_price_map(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_fetch_current_yes_price, url): url for url in urls_to_fetch
        }
        for future in as_completed(futures):
            ...
            refresh_in = max(
                ...
                POLYMARKET_PRICE_CACHE_TTL_SECONDS
                - POLYMARKET_PRICE_CACHE_REFRESH_AHEAD_SECONDS,
            )
            refresh_at = fetched_at + min(
                refresh_in, POLYMARKET_PRICE_CACHE_TTL_SECONDS
            )
...
def current_yes_prices_api(request):
    ...
    prices = {
        url: f"${price:.2f}" for url, price in price_by_url.items() if price is not None
    }
...
def _load_model_stats() -> dict[str, object]:
    ...
    for raw_line in lines:
        ...
        if (
            ...
            and "|" in line
            and not line.lower().startswith("threshold")
        ):
    ...
    summary_items = [
        ...
        {
            "label": "Best preprocessing",
            "value": summary_raw.get("Best preprocessing used", "n/a"),
        },
        {
            "label": "Validation metrics",
            "value": summary_raw.get("Validation metrics", "n/a"),
        },
        {
            "label": "Held-out test metrics",
            "value": summary_raw.get("Held-out test metrics", "n/a"),
        },
        {
            "label": "Decision threshold",
            "value": summary_raw.get("Decision threshold", "n/a"),
        },
    ]
...
def grouped_markets(request):
    ...
    for market_data in grouped.values():
        ...
        low_confidence_penalty = sum(
            max(0.0, 0.65 - confidence) for confidence in trader_confidences
        )
        ...
        market_rows.append(
            {
                ...
                "current_yes_price_usd": (
                    f"${current_yes_price:.2f}"
